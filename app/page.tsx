...
const QUESTIONS = [
  {
    ...
    terms: ["input", "memory", "state", "flip-flop"],
    ideal:
  ...
  },
  {
    ...
    terms: ["frequent", "data", "close", "cpu", "latency"],
    ideal:
  ...
  },
  {
    ...
    terms: ["event", "pause", "handler", "cpu", "polling"],
    ideal:
  ...
  },
  {
    ...
    terms: ["pin", "clock", "reset", "timing", "voltage"],
    ideal:
...
export default function Home() {
      ...
      try {
        ...
        if (latest?.question_date === today) {
          const savedResult: Result = {
            ...
            label: latest.result_label,
            hits: JSON.parse(latest.hits_json),
            misses: JSON.parse(latest.misses_json),
          };
  ...
  function submit() {
    ...
    if ((clean.length > 420 || aiSignalCount >= 2) && !rewrite) {
      setRewrite(true);
      return;
    }
    ...
    const hits = q.terms.filter((term) => low.includes(term));
    const misses = q.terms.filter((term) => !low.includes(term));
    ...
    const score = Math.min(
      ...
      Math.round(
        (
          5.2 +
          hits.length * 0.9 +
          Math.min(clean.length / 180, 1) * 1.2
        ) *
          10
      ) / 10
    );
        ...
        <section className="game-card">
          ...
          {!result ? (
            <>
              ...
              <p className="privacy">
                <Target size={15} />
                Your response is checked for
                learning—not used to accuse or
                penalize you.
              </p>
            </>
          ) : (
