...
export default function Home(){
 ...
 useEffect(()=>{const saved=localStorage.getItem("interviewdle-theme");if(saved==="dark"||saved==="light")setTheme(saved)},[]);
 useEffect(()=>{document.documentElement.dataset.theme=theme;localStorage.setItem("interviewdle-theme",theme)},[theme]);
 ...
 return <main className="min-h-screen"><header className="topbar"><div className="brand"><span className="brand-mark" aria-hidden="true"><Sun size={19}/></span><span>INTERVIEWDLE</span></div><div className="header-stats"><span><Flame size={18}/>{streak} day streak</span><span className="desktop-only"><Trophy size={17}/>{played} completed</span><div className="theme-picker" role="group" aria-label="Color theme"><button className={theme==="light"?"active":""} onClick={()=>setTheme("light")} aria-label="Use light theme" title="Light theme"><Sun size={16}/></button><button className={theme==="dark"?"active":""} onClick={()=>setTheme("dark")} aria-label="Use dark theme" title="Dark theme"><Moon size={16}/></button></div><SignedOut><SignInButton mode="modal"><button className="sign-in">Sign in</button></SignInButton></SignedOut><SignedIn><UserButton/></SignedIn></div></header>
